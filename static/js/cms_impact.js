...
export function add_parent_task_block (obj_task = null) {
  ...
  if (obj_task == null) {
    ...
    // Replace variable in str_parent_task_block
    var str_parent_task_innetHTML = str_parent_task_block.replaceAll("UUID_TASK", obj_task.uuid.toString());
    // Replace variable and generate block
    var parent_task_block = document.createElement("div");
    parent_task_block.innerHTML = str_parent_task_innetHTML;
    // Append
    ...
    // Set block data
    document.getElementById("parent_task_name_" + obj_task.uuid).value = obj_task.name;
    var list_period = [];
    try {
      list_period = obj_task.period.split("-");
    } catch (e) {}
    if (list_period.length == 2) {
      document.getElementById("parent_task_start_date_" + obj_task.uuid).value = list_period[0];
      document.getElementById("parent_task_due_date_" + obj_task.uuid).value = list_period[1];
    }
